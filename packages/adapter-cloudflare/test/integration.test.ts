...
import { composeCloudflare, toCloudflare } from "@vigil/adapter-cloudflare";
...
const sendUser = toCloudflare<User>(async (req, res) => {
  res.json({ user: req.user });
});

const sendOk = toCloudflare<User>(async (_req, res) => {
  res.json({ ok: true });
});
...
function buildApp() {
  ...
  const routes: Record<string, ReturnType<typeof composeCloudflare<User>>> = {
    "POST /login": composeCloudflare(toCloudflare(vigil.authenticate("local")), sendUser),
    "GET /dashboard": composeCloudflare(toCloudflare(vigil.requireAuth()), sendUser),
    "GET /admin": composeCloudflare(toCloudflare(vigil.requireAuth()), toCloudflare(vigil.authorize("admin")), sendOk),
    "GET /api/me": composeCloudflare(toCloudflare(vigil.authenticate("jwt", { session: false })), sendUser),
    "POST /logout": composeCloudflare(toCloudflare(vigil.logout()), sendOk),
  };
...
describe("cloudflare adapter integration", () => {
  it("logs in, sets a session cookie, and reaches a protected route", async () => {
    ...
    const dashboard = await app(
      new Request("http://localhost/dashboard", { headers: { cookie: `vigil.sid=${sessionId}` } }),
    );
  ...
  it("rejects unauthenticated access to protected routes", async () => {
    const app = buildApp();
    const res = await app(new Request("http://localhost/dashboard"));
    expect(res.status).toBe(401);
  });
  ...
  it("enforces role-based authorization", async () => {
    ...
    const res = await app(new Request("http://localhost/admin", { headers: { cookie: `vigil.sid=${sessionId}` } }));
  ...
  it("logs out and revokes the session", async () => {
    ...
    await app(
      new Request("http://localhost/logout", { method: "POST", headers: { cookie: `vigil.sid=${sessionId}` } }),
    );

    const res = await app(new Request("http://localhost/dashboard", { headers: { cookie: `vigil.sid=${sessionId}` } }));
    expect(res.status).toBe(401);
  });
  ...
  it("authenticates JWT bearer tokens on stateless routes", async () => {
    ...
    const res = await app(new Request("http://localhost/api/me", { headers: { authorization: `Bearer ${token}` } }));
    expect(res.status).toBe(200);
