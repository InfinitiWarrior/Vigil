...
import { composeBun, toBun } from "@vigil/adapter-bun";
...
const sendUser = toBun<User>(async (req, res) => {
  res.json({ user: req.user });
});

const sendOk = toBun<User>(async (_req, res) => {
  res.json({ ok: true });
});
...
function buildApp() {
  ...
  const routes: Record<string, ReturnType<typeof composeBun<User>>> = {
    "POST /login": composeBun(toBun(vigil.authenticate("local")), sendUser),
    "GET /dashboard": composeBun(toBun(vigil.requireAuth()), sendUser),
    "GET /admin": composeBun(toBun(vigil.requireAuth()), toBun(vigil.authorize("admin")), sendOk),
    "GET /api/me": composeBun(toBun(vigil.authenticate("jwt", { session: false })), sendUser),
    "POST /logout": composeBun(toBun(vigil.logout()), sendOk),
  };
